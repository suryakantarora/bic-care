...
export class CurrencyMaskDirective {
  ...
  onInputChange(event:any, backspace:any) {
    ...
    if (newVal.length === 0) {
      ...
      newVal = newVal.replace(/^(\d{0,1})(\d{0,3})/, "$1.$2"); // . can be replaced with , and vice versa
      ...
      newVal = newVal.replace(
        /^(\d{0,1})(\d{0,3})(\d{0,3})(\d{0,4})/, "$1.$2.$3.$4"
      );
    }

    // this.ngControl.valueAccessor.writeValue("$" + newVal); // $ can be changed to any currency
